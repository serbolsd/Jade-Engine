...
namespace jdEngineSDK {
  ...
  RenderRayTracingApi::onRender() {
    ...
    //for (uint32 i = 0; i < m_spheres.size() - 1; i++) {
    //  auto sp = &m_spheres[i];
    //  ImGui::DragFloat3("Position##", &sp->center[0]);
    //}
    ...
    ImGui::DragFloat("Radius##1",&sp->radius, 0.20f, 0.2f);
    ...
    ImGui::DragFloat("Radius##2", &sp->radius, 0.20f, 0.2f);
    sp->radius2 = sp->radius * sp->radius;
    ...
    ImGui::DragFloat("Radius##3", &sp->radius, 0.20f, 0.2f);
    sp->radius2 = sp->radius * sp->radius;
    ...
    ImGui::DragFloat("Radius##4", &sp->radius, 0.20f, 0.2f);
    sp->radius2 = sp->radius * sp->radius;
    ...
    ImGui::DragFloat("Radius##5", &sp->radius, 0.20f, 0.2f);
    sp->radius2 = sp->radius * sp->radius;
    ...
    ImGui::DragFloat("Radius##6", &sp->radius, 0.20f, 0.2f);
    sp->radius2 = sp->radius * sp->radius;
  ...
  RenderRayTracingApi::onCreate() {
    ...
    m_image = new unsigned char[m_imgWidth * m_imgHeight * 4];
    ...
    for (uint32 y = 0; y < m_imgHeight; ++y) {
      for (uint32 x = 0; x < m_imgWidth; ++x) {
        ...
        m_pixels[index] = Trace(JDVector3(0, 0, 0), raydir, m_spheres, 0);
        imgIndex = index * 4;
